...
const toDoObj = require("../helper/todo");
describe("ToDo Store methods", async () => {
  const toDo = new ToDoStore(toDoObj);
  beforeEach(async () => {
    toDo.createFromObject(toDoObj);
    toDo.setToDoID();
  ...
  it("should return latest ToDos.", async () => {
    const latestToDos = await ToDoStore.first(); //In first we can pass integer e.g 2 or 10 for fetching latest documents
    expect(latestToDos[0].completed).eq(toDo.completed);
  });
  ...
  it("should update ToDo with given details.", async () => {
    ...
    const updatedToDoInfo = await ToDoStore.update(params);
    expect(updatedToDoInfo.completed).eq(params.completed);
  });
